...
PidObject::PidObject() : error_(3, 0), filtered_error_(3, 0), error_deriv_(3, 0), filtered_error_deriv_(3, 0)
{
  ...
  node_priv.param<std::string>("topic_from_controller", topic_from_controller_, "control_effort"); //control_effort
  node_priv.param<std::string>("topic_from_plant", topic_from_plant_, "state"); //state
  node_priv.param<std::string>("setpoint_topic", setpoint_topic_, "setpoint"); //setpoint
  ...
  control_effort_pub_ = node.advertise<std_msgs::Float64>(topic_from_controller_, 1); //control_effort
  pid_debug_pub_ = node.advertise<std_msgs::Float64MultiArray>(pid_debug_pub_name_, 1);

  ros::Subscriber plant_sub_ = node.subscribe(topic_from_plant_, 1, &PidObject::plantStateCallback, this); //state
  ros::Subscriber setpoint_sub_ = node.subscribe(setpoint_topic_, 1, &PidObject::setpointCallback, this); //setpoint
  ...
  while( ros::ok() && !ros::topic::waitForMessage<std_msgs::Float64>(setpoint_topic_, ros::Duration(10.))) //setpoint
     ROS_WARN_STREAM("Waiting for first setpoint message.");

  while( ros::ok() && !ros::topic::waitForMessage<std_msgs::Float64>(topic_from_plant_, ros::Duration(10.))) //state
...
void PidObject::printParameters()
{
  ...
  std::cout << "Name of topic from controller: " << topic_from_controller_ << std::endl; //control_effort
  std::cout << "Name of topic from the plant: " << topic_from_plant_ << std::endl; //state
